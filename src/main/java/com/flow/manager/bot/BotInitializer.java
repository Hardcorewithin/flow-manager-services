...
*/
    ...
    public void init(){
        ...
        try {
            ...
            if (!AppProperties.USE_WEBHOOK) {
                // Default (long polling only)
                telegramBotsApi = createLongPollingTelegramBotsApi();
                telegramBotsApi.registerBot(flowManagerBot);
            } else if (!AppProperties.pathToCertificatePublicKey.isEmpty()) {
                ...
                //telegramBotsApi.registerBot(new FlowManagerBotWebhook());
            } else {
    ...
    private static TelegramBotsApi createSelfSignedTelegramBotsApi() throws TelegramApiException {
        return new TelegramBotsApi(AppProperties.pathToCertificateStore, AppProperties.CERTIFICATE_STORE_PASSWORD, AppProperties.EXTERNALWEBHOOKURL, AppProperties.INTERNALWEBHOOKURL, AppProperties.pathToCertificatePublicKey);
    }
    ...
    private static TelegramBotsApi createNoSelfSignedTelegramBotsApi() throws TelegramApiException {
        return new TelegramBotsApi(AppProperties.pathToCertificateStore, AppProperties.CERTIFICATE_STORE_PASSWORD, AppProperties.EXTERNALWEBHOOKURL, AppProperties.INTERNALWEBHOOKURL);
    }
}
